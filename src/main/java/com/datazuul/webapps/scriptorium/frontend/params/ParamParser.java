...
public class ParamParser {
    ...
    public static CropParameters getCropParameters(String cropRegion, int origWidth, int origHeight) throws InvalidParametersException {
        String[] groups = cropRegion.split(",");
        if (groups.length != 4) {
            throw new InvalidParametersException();
        }
        try {
            return new CropParameters(Integer.parseInt(groups[0]), Integer.parseInt(groups[1]),
                    Integer.parseInt(groups[2]), Integer.parseInt(groups[3]));
    ...
    public static ScaleParameters getScaleParameters(String targetSize, int origWidth, int origHeight) throws InvalidParametersException {
        ...
        if (targetSize.endsWith(",")) {
            try {
                ...
                width = (int) (Float.parseFloat(targetSize.substring(0, targetSize.length() - 1)) * origWidth);
            }
